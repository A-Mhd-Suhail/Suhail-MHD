...
const fmtDT = x => { if (!x) return '—'; try { return new Date(x).toLocaleString('en-IN', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' }); } catch (e) { return String(x); } };
...
document.addEventListener('click', e => {
  const eye = e.target.closest('[data-act="pw-eye"]');
  if (eye) { const p = document.getElementById('loginPass'); if (p) p.type = p.type === 'password' ? 'text' : 'password'; }
});
document.addEventListener('click', e => {
...
/* FIX: was "ncfution" — fatal typo that killed the entire script */
...
/* I18N now lives in js/i18n.js (LANGS, I18N, t(), applyStaticI18N, setLanguage) */
...
window.addEventListener('error', e => { try { if (typeof firebase === 'undefined') { toast('⚠️ Firebase failed to load — check that js/firebase-config.js is uploaded and your internet is on.'); return; } toast('⚠️ ' + e.message); } catch (_) {} });
...
auth.onAuthStateChanged(async user => {
  ...
  try {
    ...
    ROLE = ME.role;
    if (ME.language && I18N[ME.language]) { CUR_LANG = ME.language; localStorage.setItem('mhd_lang', CUR_LANG); fillLangSelects(); applyStaticI18N(); }
    enterApp();
  } catch (err) { toast('⚠️ ' + errMsg(err)); }
});

/* ---------- NAV ---------- */
const SVG = (d) => '<svg viewBox="0 0 24 24" class="nav-ico" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">' + d + '</svg>';
const NAV_ICONS = {
  'p-dash': SVG('<path d="M4 4h7v7H4zM13 4h7v7h-7zM4 13h7v7H4zM13 13h7v7h-7z"/>'),
  'p-upcoming': SVG('<rect x="4" y="5" width="16" height="16" rx="2"/><path d="M4 10h16M8 3v4M16 3v4"/>'),
  'p-case': SVG('<path d="M9 4h6v3H9z"/><path d="M6 7h12v13H6z"/><path d="M9 12h6M9 16h4"/>'),
  'p-meds': SVG('<path d="M10.5 20.5l-7-7a5 5 0 0 1 7-7l7 7a5 5 0 0 1-7 7z"/><path d="M8.5 8.5l7 7"/>'),
  'p-results': SVG('<path d="M9 3h6v5l4 8a4 4 0 0 1-4 5H9a4 4 0 0 1-4-5l4-8z"/><path d="M7 14h10"/>'),
  'p-appts': SVG('<rect x="4" y="5" width="16" height="16" rx="2"/><path d="M4 10h16M8 3v4M16 3v4M12 14v3l2 2"/>'),
  'p-doctors': SVG('<circle cx="12" cy="7" r="4"/><path d="M5 21c0-4 3-6 7-6s7 2 7 6"/>'),
  'p-timeline': SVG('<circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 3"/>'),
  'p-vitals': SVG('<path d="M3 12h4l2-6 4 12 2-6h6"/>'),
  'p-qr': SVG('<rect x="4" y="4" width="6" height="6"/><rect x="14" y="4" width="6" height="6"/><rect x="4" y="14" width="6" height="6"/><path d="M14 14h3v3h-3zM20 14v2M17 20h3"/>'),
  'p-privacy': SVG('<rect x="5" y="11" width="14" height="9" rx="2"/><path d="M8 11V8a4 4 0 0 1 8 0v3"/>'),
  'p-billing': SVG('<circle cx="12" cy="12" r="9"/><path d="M12 7v10"/>'),
  'EMERGENCY': SVG('<path d="M12 3l10 18H2z"/><path d="M12 10v4M12 17.5v.5"/>'),
  'd-dash': SVG('<path d="M4 4h7v7H4zM13 4h7v7h-7zM4 13h7v7H4zM13 13h7v7h-7z"/>'),
  'd-vitals': SVG('<path d="M3 12h4l2-6 4 12 2-6h6"/>'),
  'd-cases': SVG('<path d="M9 4h6v3H9z"/><path d="M6 7h12v13H6z"/><path d="M9 12h6M9 16h4"/>'),
  'd-verify': SVG('<circle cx="12" cy="12" r="9"/><path d="M8 12.5l3 3 5-6"/>'),
  'd-chats': SVG('<path d="M4 5h16v11H9l-5 4z"/><path d="M8 9h8M8 12h5"/>'),
  'd-patients': SVG('<circle cx="9" cy="8" r="3.5"/><path d="M2.5 20c0-3.5 3-5.5 6.5-5.5s6.5 2 6.5 5.5"/><circle cx="17" cy="9" r="2.5"/><path d="M16 14.5c3 0 5.5 1.8 5.5 4.5"/>'),
  'd-appts': SVG('<rect x="4" y="5" width="16" height="16" rx="2"/><path d="M4 10h16M8 3v4M16 3v4M12 14v3l2 2"/>'),
  'd-billing': SVG('<circle cx="12" cy="12" r="9"/><path d="M12 7v10"/>'),
  'h-dash': SVG('<path d="M4 21V5l8-2 8 2v16"/><path d="M12 8v6M9 11h6M9 21v-4h6v4"/>'),
  'h-patients': SVG('<circle cx="9" cy="8" r="3.5"/><path d="M2.5 20c0-3.5 3-5.5 6.5-5.5s6.5 2 6.5 5.5"/><circle cx="17" cy="9" r="2.5"/><path d="M16 14.5c3 0 5.5 1.8 5.5 4.5"/>'),
  'h-doctors': SVG('<circle cx="9" cy="8" r="3.5"/><path d="M2.5 20c0-3.5 3-5.5 6.5-5.5s6.5 2 6.5 5.5"/><circle cx="17" cy="9" r="2.5"/><path d="M16 14.5c3 0 5.5 1.8 5.5 4.5"/>'),
  'h-cases': SVG('<path d="M9 4h6v3H9z"/><path d="M6 7h12v13H6z"/><path d="M9 12h6M9 16h4"/>'),
  'h-meds': SVG('<path d="M10.5 20.5l-7-7a5 5 0 0 1 7-7l7 7a5 5 0 0 1-7 7z"/><path d="M8.5 8.5l7 7"/>'),
  'h-docs': SVG('<path d="M6 3h9l4 4v14H6z"/><path d="M9 13l2 2 4-4"/>'),
  'h-appts': SVG('<rect x="4" y="5" width="16" height="16" rx="2"/><path d="M4 10h16M8 3v4M16 3v4M12 14v3l2 2"/>'),
  'h-billing': SVG('<circle cx="12" cy="12" r="9"/><path d="M12 7v10"/>'),
  'h-upload': SVG('<path d="M4 17v3h16v-3"/><path d="M12 4v11M8 8l4-4 4 4"/>'),
  'LOGOUT': SVG('<path d="M9 4H5v16h4"/><path d="M14 8l4 4-4 4M18 12H9"/>')
};
const MENUS = {
...
function buildNav() {
  ...
  MENUS[ROLE].forEach(([id, emo, key]) => {
    ...
    b.innerHTML = (NAV_ICONS[id] || emo) + '<span>' + t(key) + '</span>'; nav.appendChild(b);
  });
  const lb = document.createElement('button'); lb.className = 'nav-item danger'; lb.dataset.nav = 'LOGOUT'; lb.innerHTML = (NAV_ICONS.LOGOUT || '🚪') + '<span>' + t('logout') + '</span>'; nav.appendChild(lb);
...
/* ----- APPOINTMENT CANCEL / COMPLETE ----- (repaired — this block was broken in your file) */
document.addEventListener('click', async e => {
  const cc = e.target.closest('[data-act="cancel-appt"]');
  if (cc) {
    try {
      const adoc = await db.collection('appointments').doc(cc.dataset.id).get();
      const a = adoc.exists ? adoc.data() : null;
      await db.collection('appointments').doc(cc.dataset.id).update({ status: 'cancelled' });
...
/* ----- TIMELINE (shows date + TIME, newest first) ----- */
function tlTime(tt) {
...
if (topSearchEl) topSearchEl.addEventListener('input', e => {
...
const cbInputEl = $('#cbInput');
if (cbInputEl) cbInputEl.addEventListener('keydown', e => { if (e.key === 'Enter') sendCB(); });
...
function renderSettings() {
  try {
    ...
    /* FIX: was $('#pg-settings') — wrong id, crashed Settings */
    mountDatePickers($('#pg-p-settings'));
...
if (dpSearchEl) dpSearchEl.addEventListener('input', renderPatients);
...
/* ----- HOSPITAL UPLOAD RESULT (Patient + Doctor dropdowns) ----- */
...
/* Patient-portal render functions now live in js/patient.js */
